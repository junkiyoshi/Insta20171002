...
void ofApp::draw(){
	...
	for (int y = 0; y < this->depth.rows; y += 10) {
		for (int x = 0; x < this->depth.cols; x += 10) {
			int z = depth.at<unsigned short>(y, x);
			if (z != 0 && z <768) {
				int height = ofMap(z, 0, 760, 512, 0);
				ofBox(ofVec3f(x, y, height / -2), 10, 10, height);
			}
			else {
				ofBox(ofVec3f(x, y, -5), 10, 10, 10);
			}
		}
